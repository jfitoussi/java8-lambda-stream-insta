...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream()).map(Item::getName).filter(name -> !itemListOnSale.contains(name)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getWantToBuy().stream().filter(item -> onSale.stream().anyMatch(saleItem -> saleItem.getName().equals(item.getName()))).map(item -> onSale.stream().filter(item1 -> item1.getName().equals(item.getName())).sorted(Comparator.comparingInt(Item::getPrice)).collect(Collectors.toList()).get(0)).mapToInt(Item::getPrice).sum() <= customer.getBudget();
