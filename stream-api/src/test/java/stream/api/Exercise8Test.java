...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(c -> c.getWantToBuy().stream().map(i -> i.getName())).filter(c -> !itemListOnSale.contains(c)).collect(Collectors.toSet());
