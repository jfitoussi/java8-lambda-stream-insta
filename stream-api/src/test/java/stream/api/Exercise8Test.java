...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = shopStream.flatMap(items -> items.getItemList().stream().map(item -> item.getName())).collect(Collectors.toList());
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream().map(item -> item.getName()).filter(item -> !itemListOnSale.contains(item))).collect(Collectors.toSet());
