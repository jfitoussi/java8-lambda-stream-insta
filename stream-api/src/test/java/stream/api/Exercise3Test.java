...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void youngestCustomer() {
         ...
         * Don't use {@link Stream#sorted}
         */
        Comparator<Customer> comparator = (Customer c1, Customer c2) -> c1.getAge().compareTo(c2.getAge());
        Optional<Customer> youngestCustomer = customerList.stream().map(customer -> customer).min(comparator);
