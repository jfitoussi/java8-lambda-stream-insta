...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void top3RichCustomer() {
         ...
         */
        Comparator<Customer> richOrder = (c1, c2) -> c2.getBudget() - c1.getBudget();
        Stream<String> top3RichCustomerStream = customerList.stream().sorted(richOrder).map(customer -> customer.getName()).limit(3);
    ...
    public void itemsCustomersWantToBuy() {
        ...
        Stream<String> itemStream = customerList.stream().flatMap(getItemStream.andThen(customer-> customer.map(item -> item.getName())));
